...
def testemails_bard():
    n1=0
    try:
        wb1 = openpyxl.load_workbook('/Users/aishwaryavijayan/Trips-aftertesting-unseen-emails.xlsx')
    ...
    objects = []
    with (open("testset-sizeok.pkl", "rb")) as openfile:
        while True:
            try:
...
try:
    wb = openpyxl.load_workbook('/Users/aishwaryavijayan/Trips-trained-may16.xlsx')
except FileNotFoundError:
...
ws5 = xw.Book("newtrips-annotat.xlsx").sheets['training']
#ws6 = xw.Book("newtrips-annotat.xlsx").sheets['training']
#v6b = ws6.range('B3').value
emails = ws5.range("B2:B40").value
#print(v6b)
prompt1 = ws5.range("D1:D40").value
prompt2 = ws5.range("E1:E40").value
prompt3 = ws5.range("F1:F40").value
#print(v6b[1])
#v6c = ws6.range('C3').value
#print(v6c)
#v5="10.eml"

#v5 = ws5.range('A3').value
#v5 = ws5.range("A1:A5").value
        ...
        if os.path.isfile(user_input + os.sep + fname):
            ...
            with open(user_input + os.sep + fname, 'rb') as f:
                ...
                if(msg.get_body(preferencelist=('plain'))):
                    ...
                    s1 = 'Retrieve all tuples of the form (Traveler-name, departure-date, departure-time, departure-location, arrival-date, arrival-time, arrival-location) in the above text. If some attributes are not known, insert null values. If there is no such tuple found in the text, please respond as NULL. I donot want any sentence to be responded. Please retrieve all trips which have at most 4 null attributes'
                    newmsg = text + s1
                    #for x in newmsg:
                     #   newmsg += ' '+ x
                    #parts = text.split(" ")
                    ...
                    #driver.type("textarea[id='mat-input-0']", mystr)
                        ...
                        if text_content!= "NULL":
                            ...
                            que1="Did you find all trips mentioned below? Please respond with just one word as either yes or no."
                            #print('v6b[0]')
                            #print(v6b[r])
                            
                            prompt1= que1 + v6b[r]

                            driver.type("textarea[id='mat-input-0']", prompt1)
                            time.sleep(60)
                            driver.click('button[mattooltip="Submit"]')
                            time.sleep(20)
                            model_response1 = driver.execute_script("return document.querySelector('.model-response-text')")
                            text_content1 = driver.execute_script("return arguments[0].textContent", model_response1)
                            print('text_content')
                            print(text_content)
                            if text_content1 == "yes":
                    ...
                    #ws.cell(row=max_row + 1, column=1).value = mystr
                                ...
                                driver.type("textarea[id='mat-input-0']", que3)
                                time.sleep(60)
                                driver.click('button[mattooltip="Submit"]')
                                time.sleep(20)
                                model_response2 = driver.execute_script("return document.querySelector('.model-response-text')")
                                text_content2 = driver.execute_script("return arguments[0].textContent", model_response2)
                                max_row = ws.max_row
                                print('text_content2')
                                print(text_content2)
                    # Write data to the next row
                    #ws.cell(row=max_row + 1, column=1).value = mystr
                                ws.cell(row=max_row + 1, column=1).value = fname
                                ws.cell(row=max_row + 1, column=2).value = text_content2
                        else:
                            que2="Please find trips of the form (Traveler-name, departure-date, departure-time, departure-location, arrival-date, arrival-time, arrival-location) similar to the below. If there are no such trips similar to the below, please respond null."
                            prompt2= que2+v6b[r]
                            driver.type("textarea[id='mat-input-0']", prompt2)
                            time.sleep(60)
                            driver.click('button[mattooltip="Submit"]')
                            time.sleep(20)
                            model_response3 = driver.execute_script("return document.querySelector('.model-response-text')")
                            text_content3 = driver.execute_script("return arguments[0].textContent", model_response3)
                            
                            
                            if text_content3 == "null":
                                que4= v6b[r]+ "is a trip. Please retrieve this trip in the form (Traveler-name, departure-date, departure-time, departure-location, arrival-date, arrival-time, arrival-location)"
                                driver.type("textarea[id='mat-input-0']", que3)
                                time.sleep(60)
                                driver.click('button[mattooltip="Submit"]')
                                time.sleep(20)
                                model_response4 = driver.execute_script("return document.querySelector('.model-response-text')")
                                text_content4 = driver.execute_script("return arguments[0].textContent", model_response4)
                                print('text_content4')
                                print(text_content4)
                                max_row = ws.max_row
                    ...
                    #ws.cell(row=max_row + 1, column=1).value = mystr
                                ws.cell(row=max_row + 1, column=1).value = fname
                                ws.cell(row=max_row + 1, column=2).value = text_content4

                
                    else:
                        print('discard'+fname)
                    r=r+1
                    path = '/Users/aishwaryavijayan/Trips-trained-may16.xlsx'
                    wb.save(path)
